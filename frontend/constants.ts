...
const RAW_COLOR_MAP: Record<string, string> = {
  "0": "#1b2a34",
  "1": "#1e5aa8",
  "2": "#00852b",
  "3": "#069d9f",
...
export const LEGO_COLOR_MAP: Record<string, string> = RAW_COLOR_MAP;

export const LEGO_COLORS: LegoColor[] = Object.entries(RAW_COLOR_MAP)
  .map(([code, hex]) => ({ code: Number(code), hex, name: `Color ${code}` }))
...
export const BRICK_HEIGHT_RATIO = LDRAW_BRICK_HEIGHT / LDRAW_UNIT_WIDTH;
export const LDRAW_PLATE_HEIGHT = 8;
export const PLATE_HEIGHT_RATIO = LDRAW_PLATE_HEIGHT / LDRAW_UNIT_WIDTH;
...
const normalizePartKey = (part: string): string => {
  ...
  return lower.includes('/') ? lower.substring(lower.lastIndexOf('/') + 1) : lower;
};
